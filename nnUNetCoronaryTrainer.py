...
from nnunetv2.training.data_augmentation.custom_transforms import CLAHETransform
...
from nnunetv2.training.loss.compound_losses import DC_and_CE_loss, DC_and_BCE_loss, soft_DC_and_clDC_loss, soft_DC_CE_and_clDC_loss, soft_CE_DC_clDC_and_HD_loss, soft_DC_CE_and_clDC_loss_new
...
class nnUNetCoronaryTrainer(nnUNetTrainer):
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            # self.loss_2 = self._build_loss_2()
            # self.loss_3 = self._build_loss_3()
    ...
    def _build_loss(self):
        ...
        if self.label_manager.has_regions:
            ...
            # loss = soft_DC_CE_and_clDC_loss({'batch_dice': batch_dice,
            #                                  'smooth': 1e-5, 'do_bg': False, 'ddp': is_ddp},
            #                                 {'batch_dice': batch_dice,
            #                                  'smooth': 1e-5, 'do_bg': False, 'ddp': is_ddp}, {}, weight_cl_dice=0.1,
            #                                 weight_dice=0.4, weight_ce=0.5,
            #                                 ignore_label=ignore_label,
            #                                 dice_class=MemoryEfficientSoftDiceLoss)
            # w_ce = max(0.5 - (0.5/self.num_epochs)*epoch, 0.1), w_dc = max(0.4 - (0.5/self.num_epochs)*epoch, 0.2), w_cl = min(0.1 + (0.5/self.num_epochs)*epoch, 0.3), w_hd = min((0.5/self.num_epochs)*epoch, 0.4)
            loss = soft_CE_DC_clDC_and_HD_loss(soft_dice_kwargs = {'batch_dice': batch_dice,
                                                 'smooth': 1e-5, 'do_bg': False, 'ddp': is_ddp},
                                                soft_cldice_kwargs = {'batch_dice': batch_dice,
                                                 'smooth': 1e-5, 'do_bg': False, 'ddp': is_ddp}, ce_kwargs={}, hd_kwargs={}, 
                                                weight_ce=0.4, weight_dice=0.3, weight_cl_dice=0.2, weight_hd=0.1, ignore_label=ignore_label, dice_class=MemoryEfficientSoftDiceLoss)
...
#                                              'smooth': 1e-5, 'do_bg': False, 'ddp': is_ddp},
#                                             {'batch_dice': batch_dice,
#                                              'smooth': 1e-5, 'do_bg': False, 'ddp': is_ddp}, {}, weight_cl_dice=0.3,
#                                             weight_dice=0.4, weight_ce=0.3,
#                                             ignore_label=ignore_label,
#                                             dice_class=MemoryEfficientSoftDiceLoss)
            
#     def _build_loss_3(self):
#         batch_dice=self.configuration_manager.batch_dice
#         is_ddp=self.is_ddp
#         ignore_label=self.label_manager.ignore_label
...
#                                              'smooth': 1e-5, 'do_bg': False, 'ddp': is_ddp},
#                                             {'batch_dice': batch_dice,
#                                              'smooth': 1e-5, 'do_bg': False, 'ddp': is_ddp}, {}, weight_cl_dice=0.5,
#                                             weight_dice=0.3, weight_ce=0.2,
#                                             ignore_label=ignore_label,
#                                             dice_class=MemoryEfficientSoftDiceLoss)
    ...
    def get_training_transforms(
        ...
        transforms.append(
            SpatialTransform(
                ...
                p_rotation=0.1, # 降低概率
                rotation=rotation_for_DA,  # 在 Z 轴上做更大角度旋转
                p_scaling=0.1, scaling=(0.7, 1.4), p_synchronize_scaling_across_axes=1,
        ...
        transforms.append(CLAHETransform(clip_limit=2.0, tile_grid_size=(8, 8), apply_probability=0.2))  #  8x8 网格内局部对比度增强
    ...
    def train_step(self, batch: dict):
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            ...
            # del data
            # if epoch_now <= 0.4*self.num_epochs:
            l = self.loss(output, target)
            # elif 0.4*self.num_epochs < epoch_now <= 0.7*self.num_epochs:
            #     l = self.loss_2(output, target)
            # else:
            #     l = self.loss_3(output, target)
